...
{
    public partial class EditCrossfit : Form
    {
        public EditCrossfit()
        {
            InitializeComponent();
        ...
        {
            string query = "SELECT COUNT(*) FROM `crossfit class` WHERE SessionId = @SessionId AND TrainerId = @TrainerId";
            using (MySqlCommand cmd = new MySqlCommand(query, conn, transaction))
            {
                cmd.Parameters.AddWithValue("@SessionId", sessionId);
                cmd.Parameters.AddWithValue("@TrainerId", trainerId);
        ...
        {
            string query = "UPDATE `crossfit class` SET TrainerId = NULL WHERE SessionId = @SessionId AND TrainerId = @TrainerId";
            using (MySqlCommand cmd = new MySqlCommand(query, conn, transaction))
            {
                cmd.Parameters.AddWithValue("@SessionId", sessionId);
                cmd.Parameters.AddWithValue("@TrainerId", trainerId);
        ...
        {
            string query = "UPDATE `crossfit class` SET TrainerId = @TrainerId WHERE SessionId = @SessionId";
            using (MySqlCommand cmd = new MySqlCommand(query, conn, transaction))
            {
                cmd.Parameters.AddWithValue("@SessionId", sessionId);
                cmd.Parameters.AddWithValue("@TrainerId", trainerId);
